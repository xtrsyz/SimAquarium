...
var aquariumConstructor = function() {
	...
	this.saveAquarium = function() {
		...
		localStorage.setItem( "usedScenery" , usedScenery );
		localStorage.setItem( "usedLight" , usedLight );
		for( i = 0; i < 9; i++ ) {
			if( sceneries[i] == true ) {
				localStorage.setItem( "sceneries" + i , "1" );
			} else {
				localStorage.setItem( "sceneries" + i , "0" );
			}
			if( lights[i] == true ) {
				localStorage.setItem( "lights" + i , "1" );
			} else {
				localStorage.setItem( "lights" + i , "0" );
			}
		}
		localStorage.setItem( "usedFilter" , usedFilter );
		for( i = 0; i < 6; i++ ) {
			if( filters[i] == true ) {
				localStorage.setItem( "filters" + i , "1" );
			} else {
				localStorage.setItem( "filters" + i , "0" );
			}
		}
		localStorage.setItem( "usedBackground" , usedBackground );

		// save the fish
		localStorage.setItem( "fishNum" , fishNum );
		for( i = 0; i < 29; i++ ) {
			localStorage.setItem( "fishNumBySpecies" + i , fishNumBySpecies[i] );
			//dbg( "saved fishnumBS: " + fishNumBySpecies[i] )
		}
		for( i = 0; i < fishNum; i++ ) {
			localStorage.setItem( "fish" + i , fish[i].serialize() );
		}
		...
		localStorage.setItem( "growHormone" , growHormone );
		localStorage.setItem( "breedHormone" , breedHormone );
		localStorage.setItem( "distraction" , distraction );
		
		localStorage.setItem( "killFish" , killFish );
		localStorage.setItem( "breedFish" , breedFish );
		
		localStorage.setItem( "fishBirths" , fishBirths );
		localStorage.setItem( "fishDeaths" , fishDeaths );
	...
	this.loadAquarium = function() {
		...
		usedScenery = parseInt( localStorage.getItem( "usedScenery" ) );
		usedLight = parseInt( localStorage.getItem( "usedLight" ) );
		for( i = 0; i < 9; i++ ) {
			var tmp = localStorage.getItem( "sceneries" + i );
			if( tmp == "1" ) {
				sceneries[i] = true;
				if( i != usedScenery ) {
			...
			var tmp = localStorage.getItem( "lights" + i );
			if( tmp == "1" ) {
				lights[i] = true;
				if( i != usedLight ) {
		...
		usedFilter = parseInt( localStorage.getItem( "usedFilter" ) );
		for( i = 0; i < 6; i++ ) {
			var tmp = localStorage.getItem( "filters" + i );
			if( tmp == "1" ) {
				filters[i] = true;
				if( i != usedFilter ) {
		...
		usedBackground = parseInt( localStorage.getItem( "usedBackground" ) )||0;
		...
		tempFishNum = parseInt( localStorage.getItem( "fishNum" ) );
		//dbg( tempFishNum );
		for( i = 0; i < 29; i++ ) {
			var tmp = localStorage.getItem( "fishNumBySpecies" + i );
			fishNumBySpecies[i] = parseInt( tmp );
		} 
		fishNum = 0;
		for( i = 0; i < tempFishNum; i++ ) {
			var serialized = localStorage.getItem( "fish" + i );
		...
		medicine = parseFloat( localStorage.getItem( "medicine" ) );
		food = parseFloat( localStorage.getItem( "food" ) ) ;
		
		growHormone = parseFloat( localStorage.getItem( "growHormone" ) );
		breedHormone = parseFloat( localStorage.getItem( "breedHormone" ) );
		distraction = parseFloat( localStorage.getItem( "distraction" ) );
		
		killFish = parseInt( localStorage.getItem( "killFish" ) );
		breedFish = parseInt( localStorage.getItem( "breedFish" ) );
	
		fishBirths = parseInt( localStorage.getItem( "fishBirths" ) );
		fishDeaths = parseInt( localStorage.getItem( "fishDeaths" ) );
